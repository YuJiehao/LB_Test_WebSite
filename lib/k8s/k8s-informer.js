...
function watchFaultState(podName, k8sClient, onChange) {
  ...
  async function connect() {
    ...
    try {
      ...
      // NOTE: watch.watch() streams ALL ConfigMap events in the
      // namespace — the fieldSelector was only applied to the initial
      // LIST call.  We MUST filter by the ConfigMap name in the event
      // handler, otherwise Pod B receives Pod A's ConfigMap changes
      // and incorrectly applies the same fault to itself.
      const req = await watch.watch(
        ...
        (type, apiObj) => {
          ...
          if (apiObj.metadata && apiObj.metadata.name !== name) return;
          onChange({
